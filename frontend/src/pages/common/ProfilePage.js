    ...
    switch (userRole) {
      ...
      case 'guest':
        return (
          <div className="mt-6 bg-white shadow overflow-hidden rounded-lg">
            <div className="px-4 py-5 sm:px-6">
              <h3 className="text-lg leading-6 font-medium text-gray-900">{t('profile.guestInformation', 'Stay Information')}</h3>
              <p className="mt-1 max-w-2xl text-sm text-gray-500">{t('profile.guestDetails', 'Your current reservation details')}</p>
            </div>
            <div className="border-t border-gray-200">
              <dl>
                <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">{t('profile.reservationType', 'Reservation Type')}</dt>
                  <dd className="mt-1 text-sm font-semibold text-primary-main sm:mt-0 sm:col-span-2">
                    {currentUser.reservationType === 'RO' && t('profile.ro', 'Room Only (RO)')}
                    {currentUser.reservationType === 'BB' && t('profile.bb', 'Bed & Breakfast (BB)')}
                    {currentUser.reservationType === 'FB' && t('profile.fb', 'Full Board (FB)')}
                    {currentUser.reservationType === 'All' && t('profile.allInclusive', 'All Inclusive (All)')}
                    {!currentUser.reservationType && t('profile.bb', 'Bed & Breakfast (BB)')} {/* Default */}
                  </dd>
                </div>
                {currentUser.roomNumber && (
                <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">{t('profile.roomNumber', 'Room Number')}</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                    {currentUser.roomNumber}
                  </dd>
                </div>
                )}
                {currentUser.checkInDate && (
                <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">{t('profile.checkInDate', 'Check-in Date')}</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                    {new Date(currentUser.checkInDate).toLocaleDateString()}
                  </dd>
                </div>
                )}
                {currentUser.checkOutDate && (
                <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                  <dt className="text-sm font-medium text-gray-500">{t('profile.checkOutDate', 'Check-out Date')}</dt>
                  <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                    {new Date(currentUser.checkOutDate).toLocaleDateString()}
                  </dd>
                </div>
                )}
              </dl>
