...
import { 
  ...
  HiFilter, HiDownload, HiStar, HiIdentification, HiCake, HiPlus, HiTrash,
  HiAnnotation, HiOfficeBuilding, HiUser, HiClock
} from 'react-icons/hi';
...
const GuestProfilePage = () => {
  ...
  const fetchGuestProfile = async () => {
    try {
      ...
      setGuest(data);
      setNotes((data.hotelNotes || []).slice().reverse()); // newest first
  ...
  const handleAddNote = async () => {
    ...
    try {
      const res = await hotelService.addGuestNote(guestId, newNoteText);
  ...
  const handleDeleteNote = async (noteId) => {
    ...
    try {
      await hotelService.deleteGuestNote(guestId, noteId);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {/* Hotel Staff Notes */}
              <div className="lg:col-span-3 rounded-[24px] overflow-hidden shadow-sm border border-gray-100 flex flex-col bg-white">
                {/* Panel header */}
                <div className="px-6 py-4 bg-gradient-to-r from-[#0b1426] to-[#1a2a4a] flex items-center justify-between">
                  <div className="flex items-center gap-2">
                ...
                <div className="px-5 py-4 border-b border-gray-100 bg-gray-50/60">
                  <div className="flex gap-3">
                    <textarea
                      ...
                      className="flex-1 text-sm border border-gray-200 rounded-xl px-4 py-2.5 resize-none focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent placeholder-gray-400 bg-white"
                    />
                    <button
                      onClick={handleAddNote}
                      disabled={isAddingNote || !newNoteText.trim()}
                      className="flex-shrink-0 flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-white text-sm font-semibold transition-all hover:opacity-90 active:scale-95 disabled:opacity-50 self-start"
                      style={{ background: 'linear-gradient(135deg, #4f46e5 0%, #0b1426 100%)' }}
                    >
                      {isAddingNote ? (
                ...
                <div className="flex-1 overflow-y-auto divide-y divide-gray-50" style={{ maxHeight: '320px' }}>
                  {notes.length === 0 ? (
                    ...
                    notes.map(note => (
                      <div
                        key={note._id}
                        className="group px-5 py-4 hover:bg-indigo-50/30 transition-colors"
                      >
                        {/* Note text */}
                        <p className="text-sm text-gray-800 leading-relaxed mb-3 whitespace-pre-wrap">{note.text}</p>
