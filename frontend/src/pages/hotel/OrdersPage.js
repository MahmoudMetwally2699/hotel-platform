...
const OrdersPage = () => {
  ...
  // Toggle provider paid status
  const toggleProviderPaid = async (bookingId) => {
    try {
      const response = await apiClient.patch(`${HOTEL_API.BOOKINGS}/${bookingId}/provider-paid`);
      if (response.data?.status === 'success') {
        // Update local state
        setBookings(prev => prev.map(b => 
          b._id === bookingId 
            ? { ...b, providerPaid: response.data.data.booking.providerPaid }
            : b
        ));
        // Update selected order if open
        if (selectedOrder?._id === bookingId) {
          setSelectedOrder(prev => ({ ...prev, providerPaid: response.data.data.booking.providerPaid }));
        }
      }
    } catch (error) {
      console.error('Error toggling provider paid status:', error);
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="flex-1 font-medium transition-colors duration-200 text-sm px-3 py-2 rounded-lg hover:bg-gray-50"
                          style={{ color: theme.primaryColor }}
                        >
                          {t('hotelAdmin.orders.details.title')}
                        </button>
