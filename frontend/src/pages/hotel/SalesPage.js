/**
 * Sales Page for Hotel Admin (المبيعات)
 * Shows hotel's sales data: totals, paid/unpaid order lists
 */
...
const SalesPage = () => {
  ...
  const fetchSalesData = useCallback(async () => {
    try {
      setLoading(true);
      const response = await hotelService.getSalesData();
  ...
  const filterOrders = (orders, search) => {
    ...
    return (orders || []).filter(order => {
      ...
      const providerName = order.serviceProviderId?.businessName?.toLowerCase() || '';
      return bookingNum.includes(s) || guestName.includes(s) || serviceName.includes(s) || category.includes(s) || providerName.includes(s);
    });
  };
  ...
  const stats = [
    {
      label: isRTL ? 'المبالغ المتبقي دفعها' : 'Remaining to Pay',
          ...
          <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                {isPaid ? (isRTL ? 'تم الدفع' : 'Paid') : (isRTL ? 'غير مدفوع' : 'Unpaid')}
              </span>
            </div>
            ...
            <p className="text-gray-400 text-xs mt-0.5">
              {order.serviceProviderId?.businessName && <span>{order.serviceProviderId.businessName} • </span>}
    ...
    <div className="min-h-screen p-4 md:p-6 lg:p-8" dir={isRTL ? 'rtl' : 'ltr'} style={{ backgroundColor: theme.backgroundColor || '#f9fafb' }}>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">
          {isRTL ? 'متابعة المبيعات والمدفوعات لمزودي الخدمات' : 'Track your sales and payments to providers'}
        </p>
      </div>
            ...
            <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <h2 className="text-base font-bold text-gray-800">
                  {isRTL ? 'العمليات غير المدفوعة' : 'Unpaid Operations'}
                </h2>
              </div>
          ...
          <div className="px-5 py-4 border-b border-gray-100" style={{ backgroundColor: `${theme.primaryColor}08` }}>
            <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
              <div className={`flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <h2 className="text-base font-bold text-gray-800">
                  {isRTL ? 'العمليات المدفوعة' : 'Paid Operations'}
                </h2>
              </div>
          ...
          <div className="p-4 space-y-3 max-h-[500px] overflow-y-auto">
            {filteredPaid.length > 0 ? (
              ...
              <div className="text-center py-10 text-gray-400">
                ...
                <p className="font-medium">{isRTL ? 'لا توجد عمليات مدفوعة' : 'No paid operations yet'}</p>
              </div>
            )}
          </div>
        </div>
