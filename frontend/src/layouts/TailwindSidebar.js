...
const TailwindSidebar = ({ isMobile, closeMobileMenu }) => {
  const { role } = useAuth();
  ...
  // Handle click on mobile to close the sidebar
  ...
  const getNavigationItems = () => {
    switch (role) {      case 'superadmin':
        return [
          ...
          { name: 'Hotels', path: '/superadmin/hotels', icon: 'building' },
          { name: 'Hotel Admins', path: '/superadmin/hotel-admins', icon: 'users' },
          { name: 'Platform Analytics', path: '/superadmin/analytics', icon: 'chart-bar' },
          { name: 'Platform Metrics', path: '/superadmin/platform-metrics', icon: 'presentation-chart-line' },
          { name: 'Settings', path: '/superadmin/settings', icon: 'cog' }
        ];      case 'hotel':
        return [
  ...
  const getIcon = (iconName) => {
    switch (iconName) {
      ...
      case 'user-add':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6m12-9a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
        );
      case 'user-group':
        return (
  ...
  return (
    <aside
      className={`bg-gray-800 text-white ${collapsed ? 'w-16' : 'w-64'} min-h-screen transition-all duration-300 ease-in-out`}
    >
